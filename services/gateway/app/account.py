...
def delete_account_logic(user, keycloak, history_api_url=None):
    username = getattr(user, "preferred_username", None) or getattr(user, "username", None) or (user if isinstance(user, str) else None)
    if not username:
        raise HTTPException(status_code=400, detail="Username not found in token or request")

    # If Keycloak is disabled (testing mode), skip Keycloak admin deletion.
    ...
    # Always attempt to delete history messages; may be unauthenticated in test mode.
    ...
    try:
        ...
        # If Keycloak is disabled, send the username explicitly so history service can
        # delete without authorization (it will accept X-Username or ?user when disabled).
        DISABLE = os.getenv("DISABLE_KEYCLOAK", "false").lower() in ("1", "true", "yes")
        if DISABLE:
            params = {"user": username}
            headers = {"X-Username": username}
            try:
                requests.delete(h_url, params=params, headers=headers, timeout=5)
            except Exception:
                pass
        else:
            try:
                requests.delete(h_url, headers=h_headers, timeout=5)
            except Exception:
                pass
    except Exception:
        pass
